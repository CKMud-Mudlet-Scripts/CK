...
cecho(
  string.format(
    ...
    math.format(API:phy_dam()),
    math.format(API:ki_dam())
  )
)
cecho(
  string.format(
    '<dim_gray>PHY DAM(B)<white>:      <yellow>%-18s<dim_gray>KI DAM(B)<white>: <yellow>%s\n',
    math.format(API:phy_dam(false, true)),
    math.format(API:ki_dam(false, true))
  )
)
cecho(
  string.format(
    '<dim_gray>PHY DAM(S)<white>:      <yellow>%-18s<dim_gray>KI DAM(S)<white>: <yellow>%s\n',
    math.format(API:phy_dam(true)),
    math.format(API:ki_dam(true))
  )
)
